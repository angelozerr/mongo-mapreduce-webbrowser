...
$(function() {
  ...
  var createMapReduce = function(file, namespaceName, name) {
    ...
    var executor = MultiPageEditor.createMapReduceEditorPage(file,
        namespaceName, name, document, mapFunc, reduceFunc);
    ...
    buttons : {
      "OK" : function() {
        ...
        if (bValid) {
          ...
          var file = $('#mapreduce_path').val();
          if (!file.endsWith('/')) {
            file += '/';
          }
          file += name;
          var namespaceName = file.replace(/\//g, '_').replace(/ /g, '_').replace(".", '_');
  ...
  // Check if there are resource parameter in the URL to open for each resource
  // the tab.
