...
SubMapReduceExecutor.prototype.execute = function(name, index) {
	...
	try {
		eval('if(!' +namespace +'){var ' + namespace + '={};}' + mapFuncName + '=' + this.mapTextarea.value);
	}
	catch(e) {
	...
	try {
		...
		var reduceResult = MR.doMapReduce(jsonArray, eval(mapFuncName), eval(reduceFuncName), finalizeFunc);
...
MapReduceExecutor.prototype.save = function() {
	...
	for ( var i = 0; i < this.executors.length; i++) {
		...
		jsContent+='\n' + reduceFuncName + '=' + executor.reduceTextarea.value;
